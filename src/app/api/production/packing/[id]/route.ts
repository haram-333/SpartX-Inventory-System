...
export async function GET(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const record = await db.collection("packing_records").findOne({ _id: new ObjectId(resolvedParams.id) })
...
export async function DELETE(request: Request, { params }: { params: { id: string } }) {
  try {
    const client = await clientPromise
    const db = client.db("SpartX-Inventory-System")
    await db.collection("packing_records").deleteOne({ _id: new ObjectId(resolvedParams.id) })
